...
(function( window, $ ){
	...
	PhotoTagger.prototype = {
		...
		// I delete the tag with the given ID. @删除
		deleteTagRecord: function( id, onSuccess ){
			var self = this;
			
			//从列表中删除该热区
			currentPage.areas.rmById(id);
		},
		...
		// I load the tag records from the server. @初始化
		loadTagRecords: function( onSuccess ){
			var self = this;
			
			// Load the tag records.
			onSuccess(currentPage.areas || []);
		},
		...
		// I save the given tag record. @保存
		saveTagRecord: function( id, x, y, width, height, message, photoID, onSuccess ){
			...
			var r_base = 10000;
			//获取图像宽高
			var imgW = $("#viewImg").width();
			var imgH = $("#viewImg").height();
			//位置比例
			area.r_x = parseInt(area.x*r_base/imgW);
			area.r_y = parseInt(area.y*r_base/imgH);
			//热区比例
			area.r_width = parseInt(area.width*r_base/imgW);
			area.r_height = parseInt(area.height*r_base/imgH);
			//3.将热区填入列表
	...
	var applyPhotoTagger = function( collection, options ){
		...
		collection.each(
			function( index, node ){
				...
				//作者水平有限，只能侵入式引用
				photoTaggerContext = photoTagger;
	...
	$.fn.photoTagger.defaultOptions = {
		...
		css: {
			"a.tag": {
				"background-image": "url( '/sys/image/transparent.gif' )",
